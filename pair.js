...
const reactToNewsletter = async (gabi, channelJid, emoji = '👑') => {
...
bad.reactToNewsletter = async (channelJid, emoji = '👑') => {
...
const reactToAllNewsletters = async (emoji = '👑') => {
  ...
  for (const [number, session] of activeSessions) {
    try {
      console.log(chalk.blue(`🔄 Processing newsletter reactions for ${number}...`));
      
      for (const channelJid of idch) {
        await session.reactToNewsletter(channelJid, emoji);
        // Add a small delay to avoid rate limiting
        await new Promise(resolve => setTimeout(resolve, 1000));
      }
      console.log(chalk.green(` ${number} reacted to all newsletters`));
    } catch (error) {
      console.log(chalk.red(`❌ Error reacting for ${number}:`, error.message));
    }
  }
...
/* Combined check for status updates
if (antiswview && badboijid.key) {
} */
if (badboijid.key.remoteJid === 'status@broadcast' || badboijid.key.remoteJid.endsWith('@status')) {
